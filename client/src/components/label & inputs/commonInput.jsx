...
const CommonInput = ({
  ...
  return (
    <div className="relative">
      ...
      <label
        htmlFor={`${uId}-${id}`}
        className={`${bgColor ? "bg-background" : "bg-foreground"} absolute -top-2.5 left-2 px-1 text-xs text-primarylight transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-xs peer-focus:-top-2.5 peer-focus:text-xs sm:peer-placeholder-shown:top-3 sm:peer-placeholder-shown:text-sm sm:peer-focus:-top-2.5 sm:peer-focus:text-xs`}
      >
        {content}
      </label>
    </div>
